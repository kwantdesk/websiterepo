...
assert.notEqual(nq?.source, "NQ", "GEX Levels must never select the retired native provider path");
...
assert.equal(normalized.showCallWall, true);
assert.equal(normalized.showGammaCentre, false);
...
const labelled = labelGexLevels(selected, 100);
assert.equal(labelled.find((row) => row.id === "wall")?.label, "Call Wall");
assert.equal(labelled.find((row) => row.id.startsWith("tail-"))?.label, "GEX Support 1");
const hiddenWalls = filterGexLevels(rows, { ...normalized, showCallWall: false });
assert.equal(hiddenWalls.some((row) => row.kind === "CALL_WALL"), false);
...
console.log("GEX Levels settings, source selection, filtering, labels, ranking, bounds and colours passed.");
