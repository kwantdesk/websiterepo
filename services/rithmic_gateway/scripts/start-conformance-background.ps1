...
if ($env:RITHMIC_APP_NAME -notlike "jahu:*") {
  throw 'RITHMIC_APP_NAME must begin with "jahu:" for conformance.'
}
...
if (Test-Path -LiteralPath $pidPath) {
  ...
  if ($existing) {
    Write-Output "Rithmic Order Plant conformance connection is already running (PID $existingPid)."
    exit 0
  }
}
...
$process = Start-Process `
  -FilePath "node" `
  -ArgumentList "scripts/conformance-order-plant.mjs" `
...
if ($process.HasExited) {
  throw "Rithmic Order Plant conformance connection exited during startup. Inspect $stderrPath"
}

Write-Output "Rithmic Order Plant conformance connection started (PID $($process.Id))."
