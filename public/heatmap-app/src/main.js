...
class DepthForgeApp {
  ...
  #updateSymbolUi() {
    ...
    const provider = String(this.liveStatus.provider || 'Databento').toUpperCase();
    ...
    if ($('footerModeStatus')) $('footerModeStatus').textContent = isDatabentoLive ? `${provider} L3 · READ-ONLY` : `CONNECTING TO ${provider}`;
    if (isDatabentoLive) {
      ...
      if ($('sourceBanner')) $('sourceBanner').textContent = `Live ${provider} depth ${condition.toLowerCase()} · full resting book · trading disabled`;
    } else {
      const state = this.liveStatus.connected ? 'BUILDING L3 BOOK' : 'CONNECTING';
      $('feedStatus').textContent = `${state} · ${provider}`;
      if ($('sourceBanner')) $('sourceBanner').textContent = this.liveStatus.message || `Connecting to live ${provider} depth for ${this.symbol}`;
    }
  }
  ...
  #ingestDatabentoSnapshot(snapshot) {
    const supportedSource = snapshot.source === 'databento-mbo' || snapshot.source === 'rithmic-depth-by-order';
    if (snapshot.symbol !== this.symbol || !supportedSource || snapshot.readOnly !== true) return;
