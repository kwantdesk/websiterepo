...
export class LiquidityStopSweepDetectorEngine {
  ...
  apply(snapshot: RithmicLiquiditySnapshot, rawSettings?: LiquidityStopSweepSettings | Record<string, unknown>, references: SweepReferenceLevel[] = []): LiquidityStopSweepFrame {
    ...
    for (const raw of trades) { const tradeId = String(raw.id ?? `${raw.timestamp}:${raw.price}:${raw.size}:${raw.side}`); if (this.seenTrades.has(tradeId)) continue; this.seenTrades.add(tradeId); this.seenTradeQueue.push(tradeId); const direction: SweepDirection = raw.side === "BUY" ? "buy" : "sell"; const trade: SweepTradeContribution = { tradeId, timestampMs: raw.timestamp, priceTick: priceToSweepTick(raw.price, this.tickSize), quantity: raw.size, aggressorSide: direction, classificationConfidence: 1, classificationMethod: "source" };
    ...
    const retentionStart = timestamp - settings.historySeconds * 1_000; for (const [id, event] of this.events) if (event.endMs < retentionStart) this.events.delete(id); this.baselines = this.baselines.filter((item) => item.timestamp >= timestamp - settings.baselineWindowMs).slice(-settings.baselineSampleLimit); while (this.seenTradeQueue.length > 100_000) this.seenTrades.delete(this.seenTradeQueue.shift()!); for (const id of this.lastAlertState.keys()) if (!this.events.has(id)) this.lastAlertState.delete(id);
