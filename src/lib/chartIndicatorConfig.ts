...
export const INDICATOR_NUMERIC_SETTINGS: Record<string, IndicatorNumericSetting[]> = {
  ...
  "parabolic-sar": [
    ...
    { key: "accelerationMaximum", label: "Acceleration maximum", defaultValue: 0.2, min: 0, max: 1, step: 0.001 },
    { key: "lineWidth", label: "Line width / point size", defaultValue: 1, min: 1, max: 4, step: 1 },
  ],
  "average-directional-index-adx": [
  ...
  "overlay-timeframe-candlestick": [
    { key: "candleWidthPercent", label: "Higher-timeframe candle width (%)", defaultValue: 94, min: 10, max: 100, step: 1 },
...
const indicatorSettingsFromTheme = (indicatorId: string, theme?: ChartSettings) => ({
  ...
  ...(indicatorId === "gamma-levels" ? {
    ...
    showPutWall: true,
    showZeroGamma: true,
    showGammaMagnet: true,
    showGammaAccelerator: true,
    showHighVolatilityLevel: true,
    showGammaCentre: false,
    showMajorLevels: true,
    showRankedLevels: true,
  ...
  ...(indicatorId === "overlay-timeframe-candlestick" ? {
    timeframe: "15m",
    filled: true,
    candleWidthPercent: 94,
    borderWidth: 1,
    opacity: 34,
    drawCloseBoundary: false,
    useThemeColors: true,
    upColor: theme?.upColor ?? "#22C55E",
    downColor: theme?.downColor ?? "#EF4444",
    closeBoundaryColor: theme?.borderUpColor ?? theme?.upColor ?? "#60A5FA",
    overlaySettingsVersion: CHART_OVERLAY_SETTINGS_VERSION,
  } : {}),
  ...(indicatorId === "deep-m-ivb" ? {
