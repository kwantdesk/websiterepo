  ...
  return (
    <div className="relative flex h-full min-h-0 flex-col overflow-hidden bg-background text-foreground">
      ...
      >
              ...
              {kind === "tpo" ? <div className="grid grid-cols-2 gap-3"><IndicatorNumericSlider label="Rows · ticks" min={1} max={64} value={draft.ticksPerRow} onChange={(value) => setDraft((current) => ({ ...current, ticksPerRow: value }))} /><IndicatorNumericSlider label="TPO period · min" min={5} max={120} step={5} value={draft.subperiodMinutes} onChange={(value) => setDraft((current) => ({ ...current, subperiodMinutes: value }))} /></div> : null}
              {kind === "volume" ? <div className="space-y-3 border border-border bg-background p-2.5">
                ...
                {draft.volumeGranularity === "auto" ? <IndicatorNumericSlider label="Target rows" min={20} max={400} step={5} value={draft.targetRows} onChange={(value) => setDraft((current) => ({ ...current, targetRows: value }))} /> : null}
                {draft.volumeGranularity === "ticks" ? <IndicatorNumericSlider label="Ticks per row" min={1} max={500} value={draft.ticksPerRow} onChange={(value) => setDraft((current) => ({ ...current, ticksPerRow: value }))} /> : null}
                {draft.volumeGranularity === "price" ? <IndicatorNumericSlider label="Price per row" min={0.000001} max={1_000} step={0.000001} value={draft.pricePerRow} onChange={(value) => setDraft((current) => ({ ...current, pricePerRow: value }))} /> : null}
              </div> : null}
              {kind === "volume" ? <div className="space-y-3 border border-border bg-background p-2.5">
              ...
              {kind === "volume" ? <div className="space-y-3 border border-border bg-background p-2.5">
                ...
                <div className="grid grid-cols-2 gap-2"><IndicatorNumericSlider label="Minimum size" min={0} max={1_000_000} value={draft.minTradeVolume} onChange={(value) => setDraft((current) => ({ ...current, minTradeVolume: value }))} /><IndicatorNumericSlider label="Maximum · 0 = none" min={0} max={1_000_000} value={draft.maxTradeVolume} onChange={(value) => setDraft((current) => ({ ...current, maxTradeVolume: value }))} /></div>
              </div> : null}
