...
import {
  buildUnfinishedAuctionFrame,
  normalizeUnfinishedAuctionSettings,
  type UnfinishedAuctionFrame,
...
import {
  ...
  type DarkPoolGexInteractionResolution,
...
function Chart({
  ...
  const stopSpotterPrimitiveRef = useRef<StopSpotterPrimitive | null>(null);
  const stopSpotterLastReactPublishRef = useRef(0);
  const stopSpotterSeenMarkerIdsRef = useRef(new Set<string>());
  const stopSpotterAlertBaselineReadyRef = useRef(false);
  const deepWallPrimitiveRef = useRef<DeepWallPrimitive | null>(null);
  ...
  const unfinishedAuctionIndicator = useMemo(
  ...
  const unfinishedAuctionSettings = useMemo(() => {
    const normalized = normalizeUnfinishedAuctionSettings(unfinishedAuctionIndicator?.settings);
    if (!normalized.useThemeColors) return normalized;
    return {
      ...normalized,
      badHighColor: settings.downColor,
  ...
  const stopSpotterSettings = useMemo(() => {
  ...
  const deepWallSettings = useMemo(() => {
    const normalized = normalizeDeepWallSettings(deepWallIndicator?.settings);
    if (!normalized.useThemeColors) return normalized;
    const visible = visibleIndicatorTheme(settings);
  ...
  useEffect(() => {
    if (!unfinishedAuctionIndicator) {
      unfinishedAuctionPrimitiveRef.current?.update(null);
      setUnfinishedAuctionFrame(null);
      return;
    }
    const frame = buildUnfinishedAuctionFrame(
      liveFootprintRenderBars,
      rawPocAuctionBars,
      instrument,
      priceFormat.minMove,
      unfinishedAuctionSettings,
    );
  ...
  useEffect(() => {
    if (!stopSpotterIndicator) {
      stopSpotterPrimitiveRef.current?.update(null);
      stopSpotterLastReactPublishRef.current = 0;
      stopSpotterSeenMarkerIdsRef.current.clear();
      stopSpotterAlertBaselineReadyRef.current = false;
      setStopSpotterFrame(null);
      return;
    }
    const frame = buildStopSpotterFrame(rawPocAuctionBars, instrument, priceFormat.minMove, stopSpotterSettings, Date.now(), timeframeToMs(timeframe));
    stopSpotterPrimitiveRef.current?.update({ frame, settings: stopSpotterSettings });
    const now = performance.now();
    if (stopSpotterLastReactPublishRef.current === 0 || now - stopSpotterLastReactPublishRef.current >= LIVE_INDICATOR_REACT_SUMMARY_INTERVAL_MS) {
      stopSpotterLastReactPublishRef.current = now;
      setStopSpotterFrame(frame);
    }
    const recentCutoff = rawPocAuctionBars.at(-2)?.startTime ?? rawPocAuctionBars.at(-1)?.startTime ?? Number.POSITIVE_INFINITY;
    if (!stopSpotterAlertBaselineReadyRef.current) {
      for (const marker of frame.markers) stopSpotterSeenMarkerIdsRef.current.add(marker.id);
      stopSpotterAlertBaselineReadyRef.current = true;
    } else {
      for (const marker of frame.markers) {
        if (stopSpotterSeenMarkerIdsRef.current.has(marker.id)) continue;
        stopSpotterSeenMarkerIdsRef.current.add(marker.id);
        if (marker.timestamp < recentCutoff || (!stopSpotterSettings.alertSoundEnabled && !stopSpotterSettings.messagePopupEnabled)) continue;
        window.dispatchEvent(new CustomEvent("kwantdesk:chart-indicator-alert", { detail: {
          indicatorId: "stop-spotter",
          instanceId: stopSpotterIndicator.instanceId,
          instrument,
          title: stopSpotterSettings.messageText || "Stop Run",
          sound: stopSpotterSettings.alertSoundEnabled ? stopSpotterSettings.alertTone : null,
          popup: stopSpotterSettings.messagePopupEnabled,
          event: marker,
        } }));
      }
    }
  }, [instrument, priceFormat.minMove, rawPocAuctionBars, stopSpotterIndicator, stopSpotterSettings, timeframe]);

  useEffect(() => {
    if (!deepWallIndicator) {
      deepWallPrimitiveRef.current?.update(null);
      deepWallLastReactPublishRef.current = 0;
      deepWallSeenMarkerIdsRef.current.clear();
      deepWallAlertBaselineReadyRef.current = false;
      setDeepWallFrame(null);
      return;
    }
    const frame = buildDeepWallFrame(rawPocAuctionBars, instrument, priceFormat.minMove, deepWallSettings);
    deepWallPrimitiveRef.current?.update({ frame, settings: deepWallSettings });
    const now = performance.now();
    if (deepWallLastReactPublishRef.current === 0 || now - deepWallLastReactPublishRef.current >= LIVE_INDICATOR_REACT_SUMMARY_INTERVAL_MS) {
      deepWallLastReactPublishRef.current = now;
      setDeepWallFrame(frame);
    }
    const recentCutoff = rawPocAuctionBars.at(-3)?.startTime ?? rawPocAuctionBars.at(-1)?.startTime ?? Number.POSITIVE_INFINITY;
    if (!deepWallAlertBaselineReadyRef.current) {
      for (const marker of frame.markers) deepWallSeenMarkerIdsRef.current.add(marker.id);
      deepWallAlertBaselineReadyRef.current = true;
    } else {
      for (const marker of frame.markers) {
        if (deepWallSeenMarkerIdsRef.current.has(marker.id)) continue;
        deepWallSeenMarkerIdsRef.current.add(marker.id);
        if (marker.confirmedAt < recentCutoff || (!deepWallSettings.alertSoundEnabled && !deepWallSettings.messagePopupEnabled)) continue;
        window.dispatchEvent(new CustomEvent("kwantdesk:chart-indicator-alert", { detail: {
          indicatorId: "deep-wall", instanceId: deepWallIndicator.instanceId, instrument,
          title: deepWallSettings.messageText || "KWANT Wall",
          sound: deepWallSettings.alertSoundEnabled ? deepWallSettings.alertTone : null,
          popup: deepWallSettings.messagePopupEnabled, event: marker,
        } }));
      }
    }
  ...
  const ivRankSubscriptions = useMemo(() => indicators.flatMap((instance) => {
    ...
    const query: IvRankQuery = {
      ...
      maximumForwardFillMinutes: Math.max(1, Math.min(60, Number(instance.settings?.maximumForwardFillMinutes ?? 5))),
      carryLastValid: instance.settings?.carryLastValid !== false,
    };
  ...
  const calculatedIndicatorSeries = useMemo(() => [
    ...
    ...indicators.flatMap((instance): CalculatedIndicatorSeries[] => {
      ...
      const rankData = dataFor((item) => mode === "call" ? item.call?.ivRank : mode === "put" ? item.put?.ivRank : item.combined?.ivRank);
      if (snapshot.current?.status === "live" && typeof snapshot.current.ivRank === "number") {
        rankData.push({ time: Math.floor(snapshot.current.timestampMs / 1_000), value: snapshot.current.ivRank });
      }
      return [{
  ...
  const calculatedIndicatorPanes = useMemo(() => {
    return indicators.flatMap((instance): IndicatorPaneGroup[] => {
      ...
      if (instance.indicatorId === "implied-volatility-rank") {
        ...
        const lineWidth = Math.max(1, Math.min(4, Math.round(Number(instance.settings?.lineWidth ?? 2)))) as 1 | 2 | 3 | 4;
        const priceLineWidth = Math.max(1, Math.min(4, Math.round(Number(instance.settings?.priceLineWidth ?? 1.5)))) as 1 | 2 | 3 | 4;
        const breakAtMissingData = instance.settings?.breakAtMissingData !== false;
        const makeData = (read: (observation: NonNullable<typeof snapshot>["observations"][number]) => number | null | undefined) => {
          let previousTimestamp = 0;
          return (snapshot?.observations ?? []).flatMap((observation) => {
          const value = read(observation);
            if (typeof value !== "number" || !Number.isFinite(value)) return [];
            const breakBefore = breakAtMissingData && previousTimestamp > 0 && observation.timestampMs - previousTimestamp > 4 * 86_400_000;
            previousTimestamp = observation.timestampMs;
            return [{ time: Math.floor(observation.timestampMs / 1_000), value, breakBefore }];
          });
        };
        const series: CalculatedIndicatorSeries[] = [];
        const mode = String(instance.settings?.contractMode ?? "average-call-put");
        if (instance.settings?.showIvRank !== false) {
          if (mode === "call-put-split") {
            ...
            const rankData = makeData(readRank);
            if (snapshot?.current?.status === "live" && typeof snapshot.current.ivRank === "number") {
              rankData.push({ time: Math.floor(snapshot.current.timestampMs / 1_000), value: snapshot.current.ivRank, breakBefore: false });
            }
            series.push({ key: `${instance.instanceId}-rank`, label: "IV Rank", kind: "line", placement: "pane", color: rankColor, lineWidth, data: rankData });
          }
        }
        ...
        if (instance.settings?.showPriceOverlay !== false) {
          series.push({
            ...
            independentScale: true,
            data: indicatorCandles.map((candle) => ({ time: Math.floor(candle.timestamp / 1_000), value: candle.close })),
          });
        }
        const currentRank = snapshot?.current?.ivRank;
        const statusDetails = deriveIvRankDisplayStatus({
          status: resource?.loading && !snapshot ? "loading" : resource?.error && !snapshot ? "unavailable" : snapshot?.overallStatus,
          ageMs: snapshot?.current?.ageMs,
          hasPriorSessionRange: Boolean(snapshot?.latestHistorical),
        });
        const status = resource?.error && snapshot ? "STALE · REFRESH DELAYED" : resource?.refreshing ? `${statusDetails.label} · REFRESHING` : statusDetails.label;
        const precision = Math.max(0, Math.min(4, Math.round(Number(instance.settings?.decimalPrecision ?? 2))));
        const lowThreshold = Number(instance.settings?.lowThreshold ?? 20);
        const middleThreshold = Number(instance.settings?.middleThreshold ?? 50);
        const highThreshold = Number(instance.settings?.highThreshold ?? 80);
        const mappedSource = snapshot?.sourceTicker ?? automaticIvSourceTicker(instrument);
        const sourceDisplay = mappedSource === instrument.toUpperCase() ? mappedSource : `${mappedSource}→${instrument.toUpperCase()}`;
        const tooltipPoints = (snapshot?.observations ?? []).map((observation) => {
          const selected = mode === "call" ? observation.call : mode === "put" ? observation.put : observation.combined;
          const warnings = observation.dataQuality.warnings.join(" · ") || "None";
          return {
            time: Math.floor(observation.timestampMs / 1_000),
            rows: [
              { label: "Session", value: observation.sessionDate },
              { label: "Source → display", value: sourceDisplay },
              { label: "IV Rank", value: typeof selected?.ivRank === "number" ? `${selected.ivRank.toFixed(precision)}%` : "Unavailable" },
              { label: "IV Percentile", value: typeof observation.ivPercentile === "number" ? `${observation.ivPercentile.toFixed(precision)}%` : "Unavailable" },
              { label: "Current IV", value: displayIvPercent(selected?.lastIv, precision) },
              { label: "Window minimum", value: displayIvPercent(selected?.windowMinimumIv, precision) },
              { label: "Window maximum", value: displayIvPercent(selected?.windowMaximumIv, precision) },
              { label: "Call IV Rank", value: typeof observation.call?.ivRank === "number" ? `${observation.call.ivRank.toFixed(precision)}%` : "Unavailable" },
              { label: "Put IV Rank", value: typeof observation.put?.ivRank === "number" ? `${observation.put.ivRank.toFixed(precision)}%` : "Unavailable" },
              { label: "Contract mode", value: mode },
              { label: "Lookback · maturity", value: `${observation.lookBackPeriodDays}D · ${observation.targetMaturityDays}D` },
              { label: "Expiration", value: observation.expirationDate },
              { label: "Provider · status", value: `QuantData · ${observation.status}` },
              { label: "Quality", value: `${observation.dataQuality.score}%` },
        ...
        return [{
          ...
          showHeader: instance.settings?.showHeader !== false,
          showLegend: instance.settings?.showLegend !== false,
          percentageAxis: true,
          secondaryAxisSeriesKey: `${instance.instanceId}-price`,
          secondaryAxisLabel: instance.settings?.showPriceOverlay === false ? undefined : instrument.toUpperCase(),
          defaultHeight: Math.max(120, Math.min(520, Number(instance.settings?.paneHeight ?? 220))),
          minimumHeight: 120,
          maximumHeight: 520,
          tooltipPoints,
          currentBadgeValue: typeof currentRank === "number" ? currentRank : undefined,
          currentBadge: instance.settings?.showCurrentBadge !== false && typeof currentRank === "number" ? `IVR ${currentRank.toFixed(precision)}% · ${statusDetails.shortLabel}` : undefined,
          bands: instance.settings?.showRegimeBands === false ? [] : [
            { from: 0, to: lowThreshold, color: String(instance.settings?.lowBandColor ?? settings.downColor), opacity: 0.05 },
            { from: lowThreshold, to: middleThreshold, color: String(instance.settings?.middleBandColor ?? settings.gridColor), opacity: 0.025 },
            { from: middleThreshold, to: highThreshold, color: String(instance.settings?.middleBandColor ?? settings.gridColor), opacity: 0.05 },
            { from: highThreshold, to: 100, color: String(instance.settings?.highBandColor ?? settings.upColor), opacity: 0.07 },
          ],
  ...
  const resolvedIndicatorPaneHeights = useMemo(
    () => Object.fromEntries(calculatedIndicatorPanes.map((group) => [
      group.key,
      indicatorPaneHeights[group.key] ?? group.defaultHeight ?? defaultIndicatorPaneHeight,
    ])),
  ...
  // During session replay the heat surface follows the replay clock at the
  // same granularity as candles. A candle is fully revealed the moment the
  // clock enters its bar, so heat frames captured inside that same bar must
  // be revealed with it — gating them on the raw clock left the newest heat
  // column one bar behind the price, which never happens live. Rounding the
  // boundary up to the bar close keeps heat inline with the forming candle
  // without leaking anything beyond what the revealed candle already shows,
  // and re-filters once per bar instead of on every 200ms clock tick.
  const replayHeatBoundaryMs = useMemo(() => {
    if (!replayTimestampMs) return null;
    const intervalMs = timeframeToMs(timeframe) ?? 60_000;
    return Math.ceil(replayTimestampMs / intervalMs) * intervalMs;
  }, [replayTimestampMs, timeframe]);
  const gammaHeatmapPrimitiveData = useMemo<GammaHeatmapPrimitiveData | null>(() => {
    ...
    // The level overlay is derived from the latest live surface, so it stays
    // hidden inside a replay.
    const replayVisibleSnapshots = replayHeatBoundaryMs
      ? gammaHeatmapPayload.snapshots.filter((snapshot) => snapshot.timestamp <= replayHeatBoundaryMs)
      : gammaHeatmapPayload.snapshots;
    if (replayHeatBoundaryMs && !replayVisibleSnapshots.length) return null;
    return {
      snapshots: replayVisibleSnapshots,
      levels: replayHeatBoundaryMs ? [] : gammaHeatmapPayload.levels,
      ...
      showLevels: !replayHeatBoundaryMs && indicatorSettings.showLevels !== false,
  ...
  }, [gammaHeatmapIndicator, gammaHeatmapPayload, priceFormat.precision, replayHeatBoundaryMs, settings.backgroundColor, settings.downColor, settings.gridColor, settings.upColor]);
  useEffect(() => {
  ...
  const gexIntervalPrimitiveData = useMemo<GexIntervalMapPrimitiveData | null>(() => {
    ...
    return {
      ...
      maximumRadius: Math.max(3, Number(indicatorSettings.maximumRadius ?? 18)),
      ...
      hollowBubbles: indicatorSettings.hollowBubbles !== false,
      bubbleStrokeWidth: Math.max(0.5, Number(indicatorSettings.bubbleStrokeWidth ?? 1.25)),
      bubbleFillStrength: Math.max(0, Math.min(1, Number(indicatorSettings.bubbleFillStrength ?? 12) / 100)),
      showLevelTracks: indicatorSettings.showLevelTracks !== false,
      showUnderlyingPriceLine: indicatorSettings.showUnderlyingPriceLine === true,
      trackWidth: Math.max(0.5, Number(indicatorSettings.trackWidth ?? 1.5)),
  ...
  useEffect(() => {
    ...
    const load = async (force = false, replayAsOfMs: number | null = null, checkpoint = false) => {
      ...
      const query = new URLSearchParams({
        ...
        topExposurePercent: String(indicatorSettings.topExposurePercent),
  ...
  const resizeIndicatorPane = useCallback((key: string, nextHeight: number) => {
    const group = calculatedIndicatorPanes.find((pane) => pane.key === key);
    const height = Math.max(group?.minimumHeight ?? 64, Math.min(group?.maximumHeight ?? 420, Math.round(nextHeight)));
    setIndicatorPaneHeights((current) => ({
      ...current,
      [key]: height,
    }));
    if (group?.indicatorId === "implied-volatility-rank") updateIndicatorSettingRef.current?.(key, "paneHeight", height);
  ...
  const sessionHighLowRenderData = useMemo<SessionHighLowRenderLevel[]>(() => {
    ...
    if (initialBalanceSettings.showFib === true && initialBalanceLevels.length) {
      ...
      for (const level of initialBalanceLevels) {
        const key = `${level.session.key}-${level.session.startTimestamp}`;
  ...
  const environmentPositionStyle = (position: string, scale: number) => {
    ...
    // The old left rail is gone, so nothing docks on the left any more and the
    // plot inset is the plain margin.
    const leftInsetPx = unzoom(12);
  ...
  useEffect(() => {
    ...
    const stopSpotterPrimitive = new StopSpotterPrimitive();
    candleSeries.attachPrimitive(stopSpotterPrimitive);
    stopSpotterPrimitiveRef.current = stopSpotterPrimitive;
    ...
    return () => {
      ...
      if (candleSeriesRef.current && absorptionPrimitiveRef.current) {
        ...
        if (candleSeriesRef.current && stopSpotterPrimitiveRef.current) {
          try { candleSeriesRef.current.detachPrimitive(stopSpotterPrimitiveRef.current); } catch { /* chart may already be disposed */ }
        }
  ...
  useEffect(() => {
    ...
    const latestAnchor = () => {
      ...
      for (let index = currentCandles.length - 1; index >= 0; index -= 1) {
        const close = Number(currentCandles[index]?.close);
    ...
    const applySnapshot = (snapshot: ChartViewportSnapshot) => {
      ...
      try {
        ...
        if (Number.isFinite(targetRange.from) && Number.isFinite(targetRange.to) && targetRange.to > targetRange.from) {
    ...
    const models = volumeProfiles.flatMap((profile): NativeVolumeProfileModel[] => {
      ...
      return [{
        ...
        style: {
          ...
          pocHighlightOpacity: clamp(Number(profileSettings.pocHighlightOpacity ?? 72), 2, 100),
          shiftedPocTicks: clamp(Number(profileSettings.shiftedPocTicks ?? 1), 1, 100),
          shiftedPocOpacity: clamp(Number(profileSettings.shiftedPocOpacity ?? 70), 2, 100),
    ...
    <div className="flex h-full w-full min-w-0 overflow-hidden">
      ...
      {unfinishedAuctionIndicator ? (
        <div
          ...
          style={{ top: pocAuctionIndicator && pocAuctionSettings.showHeader ? 38 : 8 }}
          title="Exact one-tick unfinished-auction extremes from classified Rithmic executions"
        >
          <span className={`h-1.5 w-1.5 rounded-full ${!unfinishedAuctionFrame || unfinishedAuctionFrame.status === "WAITING_FOR_VOLUME_AT_PRICE" ? "animate-pulse bg-warning" : unfinishedAuctionFrame.status === "GROUPED_EXTREMES" ? "bg-warning" : "bg-primary"}`} />
          <span className="text-foreground">Unfinished Auction</span>
      ...
      {stopSpotterIndicator ? (
        <div className="pointer-events-none absolute right-2 z-[72] flex items-center gap-2 border border-border bg-panel/92 px-2 py-1 font-mono text-[8px] uppercase tracking-[0.08em] text-muted shadow-lg backdrop-blur" style={{ top: (pocAuctionIndicator && pocAuctionSettings.showHeader ? 38 : 8) + (unfinishedAuctionIndicator ? 26 : 0) + (barPocIndicator ? 26 : 0) + (dynamicPocIndicator ? 26 : 0) + (ratioHighlightIndicator ? 26 : 0) }} title="Stop-run candidates requiring the complete DeepCharts parameter contract on exact Rithmic volume-at-price">
          <span className={`h-1.5 w-1.5 rounded-full ${!stopSpotterFrame || stopSpotterFrame.status === "WAITING_FOR_VOLUME_AT_PRICE" ? "animate-pulse bg-warning" : "bg-primary"}`} />
          <span className="text-foreground">Stop Spotter</span><span>{stopSpotterFrame?.status.replaceAll("_", " ") ?? "CALCULATING"}</span><span>{stopSpotterFrame?.markers.length ?? 0} RUNS</span>
        </div>
      ) : null}
      {deepWallIndicator ? (
      ...
      {bounceLevelsTooltip ? (
        ...
        >
          <div className="flex items-center justify-between gap-4 border-b border-border pb-1 text-foreground">
            <span>{new Date(bounceLevelsTooltip.node.timestamp).toLocaleTimeString()}</span>
            <span>{bounceLevelsTooltip.node.mappedPrice.toFixed(priceFormat.precision)}</span>
          </div>
          <div className="mt-1 grid grid-cols-2 gap-x-4 gap-y-1 text-muted">
      ...
      {darkPoolGexTooltip ? (
        ...
        >
          {darkPoolGexTooltip.event ? (
            <>
              <div className="flex items-center justify-between gap-4 border-b border-border pb-1 text-foreground"><span>Dark Pool (GEX) · #{darkPoolGexTooltip.event.rank}</span><span>{darkPoolGexTooltip.event.classification.replaceAll("_", " ")}</span></div>
              <div className="mt-1 grid grid-cols-2 gap-x-4 gap-y-1 text-muted">
                <span>Execution time</span><span className="text-foreground">{new Date(darkPoolGexTooltip.event.executionTimestampMs).toLocaleString()}</span>
                <span>Report time</span><span className="text-foreground">{darkPoolGexTooltip.event.reportTimestampMs ? new Date(darkPoolGexTooltip.event.reportTimestampMs).toLocaleString() : "Not supplied"}</span>
                <span>Observable from</span><span className="text-foreground">{new Date(darkPoolGexTooltip.event.observableTimestampMs).toLocaleString()}</span>
                <span>Exact stored price</span><span className="text-foreground">{String(darkPoolGexTooltip.event.price)}</span>
                <span>Display price</span><span className="text-foreground">{darkPoolGexTooltip.event.price.toFixed(priceFormat.precision)}</span>
                <span>Original print price</span><span className="text-foreground">{String(darkPoolGexTooltip.event.print.rawPrice ?? darkPoolGexTooltip.event.sourcePrice)}</span>
                {(darkPoolGexTooltip.event.print.corporateActionAdjustmentFactor ?? 1) !== 1 ? <><span>Adjustment</span><span className="text-foreground">×{darkPoolGexTooltip.event.print.corporateActionAdjustmentFactor} · {darkPoolGexTooltip.event.print.corporateAction ?? "Corporate action"}</span></> : null}
                ...
                <span>Source / record</span><span className="text-foreground">{darkPoolGexTooltip.event.print.source ?? "QuantData"} · {darkPoolGexTooltip.event.print.recordId ?? darkPoolGexTooltip.event.print.id}</span>
                <span>TRF / ATS</span><span className="text-foreground">{darkPoolGexTooltip.event.print.trf ?? darkPoolGexTooltip.event.print.venue ?? "Not identified"}</span>
                <span>Conditions</span><span className="text-foreground">{darkPoolGexTooltip.event.print.tradeConditions?.join(", ") || "None supplied"}</span>
                <span>Record state</span><span className="text-foreground">{darkPoolGexTooltip.event.print.correctionState ?? "ORIGINAL"} · {darkPoolGexTooltip.event.print.cancellationState ?? "ACTIVE"}</span>
      ...
      {darkPoolGexIndicator?.settings?.showInspector === true && darkPoolGexFrame?.rawEvents.length ? (
        <div className="absolute bottom-10 right-[68px] z-[26] max-h-[42%] w-[min(680px,68%)] overflow-auto border border-border bg-panel/96 font-mono text-[8px] shadow-2xl">
          <div className="sticky top-0 grid grid-cols-[28px_120px_1fr_80px_86px_78px] gap-2 border-b border-border bg-panel px-2 py-1 uppercase tracking-[0.08em] text-muted"><span>Rank</span><span>Exact price / date</span><span>Notional</span><span>Distance</span><span>Touches H/B/R</span><span>GEX</span></div>
          {[...darkPoolGexFrame.rawEvents].sort((left, right) => {
            const mode = String(darkPoolGexIndicator.settings?.sortMode ?? "notional");
            if (mode === "distance" && darkPoolGexCurrentPrice !== null) return Math.abs(left.price - darkPoolGexCurrentPrice) - Math.abs(right.price - darkPoolGexCurrentPrice);
            if (mode === "freshness") return right.observableTimestampMs - left.observableTimestampMs;
            if (mode === "reaction-quality") return (right.reaction?.holds ?? 0) + (right.reaction?.reclaims ?? 0) - (right.reaction?.breaks ?? 0) - ((left.reaction?.holds ?? 0) + (left.reaction?.reclaims ?? 0) - (left.reaction?.breaks ?? 0));
            return left.rank - right.rank;
          }).map((event) => (
            <div key={event.id} className="grid grid-cols-[28px_120px_1fr_80px_86px_78px] gap-2 border-b border-border/45 px-2 py-1 text-foreground">
              <span>{event.rank}</span>
              <span>{String(event.price)} · {new Date(event.executionTimestampMs).toLocaleDateString()}</span>
              <span>{event.notional.toLocaleString("en-US", { notation: "compact", style: "currency", currency: "USD" })}</span>
              <span>{darkPoolGexCurrentPrice === null ? "—" : `${event.price >= darkPoolGexCurrentPrice ? "+" : ""}${(event.price - darkPoolGexCurrentPrice).toFixed(priceFormat.precision)}`}</span>
              <span>{event.reaction ? `${event.reaction.touchCount} · ${event.reaction.holds}/${event.reaction.breaks}/${event.reaction.reclaims}` : "—"}</span>
      ...
      {darkPoolGexIndicator?.settings?.showReactionResearch === true && darkPoolGexResearch ? (
        <div className="absolute bottom-10 left-2 z-[26] w-[min(560px,52%)] border border-border bg-panel/96 p-2 font-mono text-[8px] shadow-2xl">
          <div className="flex items-center justify-between border-b border-border pb-1 uppercase tracking-[0.1em]"><span className="text-foreground">DP Reaction Research</span><span className={darkPoolGexResearch.sufficientSample ? "text-primary" : "text-warning"}>{darkPoolGexResearch.sufficientSample ? "Sample qualified" : `Small sample · minimum ${darkPoolGexFrame?.settings.minimumResearchSamples ?? 10}`}</span></div>
          <div className="mt-2 grid grid-cols-4 gap-x-3 gap-y-1 text-muted">
            <span>Levels</span><span className="text-foreground">{darkPoolGexResearch.levelCount}</span><span>Touches</span><span className="text-foreground">{darkPoolGexResearch.touchCount}</span>
            <span>Median error</span><span className="text-foreground">{darkPoolGexResearch.medianTouchError?.toFixed(priceFormat.precision + 2) ?? "—"}</span><span>Error ticks</span><span className="text-foreground">{darkPoolGexResearch.disclosures.resolution === "tick" ? darkPoolGexResearch.medianTouchErrorTicks?.toFixed(2) ?? "—" : "Not tick-resolved"}</span>
            <span>Hold rate</span><span className="text-foreground">{darkPoolGexResearch.holdRate === null ? "—" : `${(darkPoolGexResearch.holdRate * 100).toFixed(1)}%`}</span><span>Break rate</span><span className="text-foreground">{darkPoolGexResearch.breakRate === null ? "—" : `${(darkPoolGexResearch.breakRate * 100).toFixed(1)}%`}</span>
            <span>Reclaim rate</span><span className="text-foreground">{darkPoolGexResearch.reclaimRate === null ? "—" : `${(darkPoolGexResearch.reclaimRate * 100).toFixed(1)}%`}</span><span>Median reaction</span><span className="text-foreground">{darkPoolGexResearch.medianReaction?.toFixed(priceFormat.precision) ?? "—"}</span>
            <span>Median MFE / MAE</span><span className="text-foreground">{darkPoolGexResearch.medianMfe?.toFixed(priceFormat.precision) ?? "—"} / {darkPoolGexResearch.medianMae?.toFixed(priceFormat.precision) ?? "—"}</span><span>$1B+ levels / touches</span><span className="text-foreground">{darkPoolGexResearch.billionPlus.levels} / {darkPoolGexResearch.billionPlus.touches}</span>
            <span>Fresh first touch</span><span className="text-foreground">{darkPoolGexResearch.freshFirstTouch.levels} / {darkPoolGexResearch.freshFirstTouch.touches}</span><span>DP + KING / no GEX</span><span className="text-foreground">{darkPoolGexResearch.withKing.levels} / {darkPoolGexResearch.withoutGex.levels}</span>
          </div>
          <div className="mt-2 border-t border-border pt-1 leading-4 text-muted">Tolerance {darkPoolGexResearch.disclosures.touchTolerance} · break {darkPoolGexResearch.disclosures.breakThreshold} · reaction {darkPoolGexResearch.disclosures.reactionThreshold} · horizon {Math.round(darkPoolGexResearch.disclosures.postTouchHorizonMs / 60_000)}m · resolution {darkPoolGexResearch.disclosures.resolution} · {darkPoolGexResearch.disclosures.sessionFilter} · {darkPoolGexResearch.sampleCount} level samples</div>
        </div>
      ) : null}
      {darkPoolMapIndicator ? (
        <div
