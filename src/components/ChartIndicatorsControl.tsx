            ...
            <IndicatorSettingsSections key={settingsDefinition.id}>
              ...
              {VOLUME_PROFILE_INDICATOR_IDS.has(settingsDefinition.id) && volumeProfileTab === "sessions" ? (
                <div className="grid gap-3 border border-primary/15 bg-primary/[0.035] p-3 sm:grid-cols-2">
                  ...
                  {([
                    /* Weekly defaults to a rolling five-session calculation. */
                    ...(settingsDefinition.id === "weekly-volume-profile" ? [[
                      "Calculation window", "weekSelection", "rolling-five", [
                        ["rolling-five", "Last 5 trading days · automatic"],
              ...
              {settingsDefinition.id === "parabolic-sar" ? (
                <div data-settings-section="Style" className="space-y-3">
                  {[
                    { key: "displayStyle", label: "Display style", fallback: "points", options: [["points", "Points"], ["line", "Line"], ["line-points", "Line and points"]] },
                    ...
                    <label key={control.key} className="block space-y-1 text-[10px] text-muted">
                      ...
                      <KwantSelect value={String(settingsInstance.settings?.[control.key] ?? control.fallback)}
                        ...
                        }))} menuLabel={`SAR ${control.label.toLowerCase()}`}
                        className="h-9 w-full border border-border bg-background px-3 text-foreground">
                ...
                <div className="space-y-2 rounded-lg border border-border bg-surface/30 p-2.5">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {([
                      ...
                      ["showSizes", "Contract counts"], ["alignLeft", "Both face left"],
                      ["useThemeColors", "Website colours"],
                    ] as const).map(([key, label]) => (
                      <button
                        key={key}
                        type="button"
                  ...
                  <p className="text-[8px] leading-4 text-muted">
                    The same resting-book ladder the liquidity map draws down its price axis: bid and ask size summed
                    into price bands, one thick bar per band with its contract count on it. Widen the level spacing for
                    fewer, heavier bars.
                  </p>
                </div>
              ) : null}
              ...
              {settingsDefinition.id === "shift-candle" ? (() => {
                ...
                return (
                  <div data-settings-section="Signal" className="space-y-3 rounded-xl border border-primary/15 bg-primary/[0.035] p-3">
                    <div className="grid gap-3 sm:grid-cols-2">
                      ...
                      <label className="space-y-1.5 text-[9px] uppercase tracking-[0.12em] text-muted"><span>Alert name</span><input value={String(settingsInstance.settings?.alertName ?? "Signal Trigger")} onChange={(event) => update({ alertName: event.target.value })} className="h-9 w-full rounded-lg border border-border bg-background px-3 text-[10px] normal-case tracking-normal text-foreground outline-none focus:border-primary/45" /></label>
                      <label className="space-y-1.5 text-[9px] uppercase tracking-[0.12em] text-muted"><span>Popup message</span><input value={String(settingsInstance.settings?.popupMessage ?? "Signal Trigger")} onChange={(event) => update({ popupMessage: event.target.value })} className="h-9 w-full rounded-lg border border-border bg-background px-3 text-[10px] normal-case tracking-normal text-foreground outline-none focus:border-primary/45" /></label>
                    </div>
