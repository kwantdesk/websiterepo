...
function MiniSelect({ label, value, values, onChange }: { label: string; value: string; values: string[]; onChange: (value: string) => void }) {
  return <label className="flex h-7 items-center border border-border bg-background"><span className="px-2 text-[7px] uppercase tracking-[.11em] text-muted">{label}</span><KwantSelect value={value} onChange={(event) => onChange(event.target.value)} className="h-full min-w-20 border-l border-border bg-background px-1 font-mono text-[8px] text-foreground outline-none">{values.map((item) => <option key={item}>{item}</option>)}</KwantSelect></label>;
}
...
function PanelSettingsDialog({ panel, onChange, onClose }: { panel: DashboardPanel; onChange: (panel: DashboardPanel) => void; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-[180] pointer-events-none"><div className="pointer-events-auto absolute left-1/2 top-1/2 w-[430px] max-w-[calc(100vw-24px)] -translate-x-1/2 -translate-y-1/2 border border-border bg-panel shadow-2xl" style={{ marginLeft: position.x, marginTop: position.y }}>
    ...
    <div className="grid max-h-[70vh] grid-cols-2 gap-3 overflow-y-auto p-4 text-[9px]">
      {uses.symbol ? <Field label="Symbol"><KwantSelect value={panel.settings.symbol} onChange={(e) => update("symbol", e.target.value)}>{["SPX", "SPXW", "SPY", "NDX", "QQQ"].map((v) => <option key={v}>{v}</option>)}</KwantSelect></Field> : null}
      ...
      {uses.aggregation ? <Field label="Aggregation"><KwantSelect value={panel.settings.aggregation} onChange={(e) => update("aggregation", e.target.value)}>{["1m", "2m", "3m", "4m", "5m", "10m", "15m", "20m", "30m", "1h", "2h", "4h"].map((v) => <option key={v}>{v}</option>)}</KwantSelect></Field> : null}
      {uses.greek ? <Field label="Greek"><KwantSelect value={panel.settings.greek} onChange={(e) => update("greek", e.target.value)}>{["GEX", "DEX", "VEX", "CHEX"].map((v) => <option key={v}>{v}</option>)}</KwantSelect></Field> : null}
      {uses.expiry ? <Field label="Expiration"><KwantSelect value={panel.settings.expiry} onChange={(e) => update("expiry", e.target.value)}>{["0DTE", "FRONT", "0-7DTE", "ALL"].map((v) => <option key={v}>{v}</option>)}</KwantSelect></Field> : null}
      ...
      {flowTool ? <>
        ...
        <Field label="Sentiment"><KwantSelect value={panel.settings.flowSentiment} onChange={(e) => update("flowSentiment", e.target.value)}>{["ALL", "BULLISH", "BEARISH", "NEUTRAL"].map((v) => <option key={v}>{v}</option>)}</KwantSelect></Field>
        <Field label="Aggressor side"><KwantSelect value={panel.settings.flowSide} onChange={(e) => update("flowSide", e.target.value)}>{["ALL", "ABOVE_ASK", "ASK", "MID", "BID", "BELOW_BID", "UNKNOWN"].map((v) => <option key={v}>{v}</option>)}</KwantSelect></Field>
        <Field label="Trade type"><KwantSelect value={panel.settings.flowType} onChange={(e) => update("flowType", e.target.value)}>{["ALL", "SWEEP", "BLOCK", "SPLIT", "MULTI_LEG", "EXTENDED", "CANCEL"].map((v) => <option key={v}>{v}</option>)}</KwantSelect></Field>
        ...
        <Field label="Classification"><KwantSelect value={panel.settings.flowFlags} onChange={(e) => update("flowFlags", e.target.value)}>{["ALL", "UNUSUAL", "OPENING", "GOLDEN", "VOL_GT_OI", "SIZE_GT_OI"].map((v) => <option key={v}>{v}</option>)}</KwantSelect></Field>
        <Field label="Column preset"><KwantSelect value={panel.settings.flowColumns} onChange={(e) => update("flowColumns", e.target.value)}>{["full", "essential", "execution", "contract"].map((v) => <option key={v}>{v}</option>)}</KwantSelect></Field>
        <Field label="Group rows"><KwantSelect value={panel.settings.flowGrouping} onChange={(e) => update("flowGrouping", e.target.value)}>{["none", "contract", "expiry", "exchange"].map((v) => <option key={v}>{v}</option>)}</KwantSelect></Field>
        ...
        <Field label="Sort by"><KwantSelect value={panel.settings.flowSort} onChange={(e) => update("flowSort", e.target.value)}>{[["tradeTime", "Time"], ["premium", "Premium"], ["size", "Quantity"], ["flowScore", "Flow score"], ["strikePrice", "Strike"]].map(([value, label]) => <option key={value} value={value}>{label}</option>)}</KwantSelect></Field>
        <Field label="Direction"><KwantSelect value={panel.settings.flowDirection} onChange={(e) => update("flowDirection", e.target.value)}><option value="desc">Descending</option><option value="asc">Ascending</option></KwantSelect></Field>
        <Field label="Row density"><KwantSelect value={panel.settings.density} onChange={(e) => update("density", e.target.value)}><option value="compact">Compact</option><option value="comfortable">Comfortable</option></KwantSelect></Field>
      </> : null}
      {intervalTool ? <>
        ...
        <Field label="Visual mode"><KwantSelect value={panel.settings.intervalVisual} onChange={(e) => update("intervalVisual", e.target.value)}>{["bubbles", "fixed-dots", "heat-cells", "horizontal-ribbons", "hybrid"].map((v) => <option key={v}>{v}</option>)}</KwantSelect></Field>
        <Field label="Underlying style"><KwantSelect value={panel.settings.intervalPriceStyle} onChange={(e) => update("intervalPriceStyle", e.target.value)}><option value="line">Line</option><option value="candles">Candlesticks</option></KwantSelect></Field>
        ...
        <Field label="Exposure content"><KwantSelect value={panel.settings.intervalContent} onChange={(e) => update("intervalContent", e.target.value)}>{["net", "call", "put", "gross", "call-put-split"].map((v) => <option key={v}>{v}</option>)}</KwantSelect></Field>
        <Field label="Value mode"><KwantSelect value={panel.settings.intervalMode} onChange={(e) => update("intervalMode", e.target.value)}><option value="raw">Raw exposure</option><option value="difference">Build / unwind change</option></KwantSelect></Field>
        <Field label="Difference baseline"><KwantSelect value={panel.settings.intervalBaseline} onChange={(e) => update("intervalBaseline", e.target.value)}><option value="previous-bucket">Previous interval</option><option value="session-open">Session open</option><option value="rolling-average">Rolling average</option></KwantSelect></Field>
        ...
        <Field label="Price path"><KwantSelect value={panel.settings.intervalShowPrice ? "ON" : "OFF"} onChange={(e) => onChange({ ...panel, settings: { ...panel.settings, intervalShowPrice: e.target.value === "ON" } })}><option>ON</option><option>OFF</option></KwantSelect></Field>
      </> : null}
    </div>
