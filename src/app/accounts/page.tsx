...
export default function AccountsPage() {
  ...
  return (
    <div className="flex min-h-screen bg-background text-foreground">
      ...
      {showDemoModal && <Modal title="New Demo Account" onClose={() => setShowDemoModal(false)}><div className="space-y-3"><input value={newAccountName} onChange={(event) => setNewAccountName(event.target.value)} className="w-full rounded-xl border border-border bg-surface px-4 py-3 text-[13px] outline-none" placeholder="Account name" /><select value={newAccountBalance} onChange={(event) => setNewAccountBalance(event.target.value)} className="w-full rounded-xl border border-border bg-surface px-4 py-3 text-[13px]"><option>$10,000</option><option>$1,000</option><option>$5,000</option><option>$25,000</option><option>$50,000</option><option>$100,000</option></select><select value={newAccountInstrument} onChange={(event) => setNewAccountInstrument(event.target.value)} className="w-full rounded-xl border border-border bg-surface px-4 py-3 text-[13px]"><option>NAS100</option><option>XAUUSD</option><option>BTCUSD</option><option>Multiple</option></select><select value={newAccountLeverage} onChange={(event) => setNewAccountLeverage(event.target.value)} className="w-full rounded-xl border border-border bg-surface px-4 py-3 text-[13px]"><option>1:30</option><option>1:1</option><option>1:10</option><option>1:50</option><option>1:100</option><option>1:200</option><option>1:500</option></select><select value={newAccountStrategy} onChange={(event) => setNewAccountStrategy(event.target.value)} className="w-full rounded-xl border border-border bg-surface px-4 py-3 text-[13px]"><option>Manual / No Strategy</option>{strategies.map((strategy) => <option key={strategy}>{strategy}</option>)}</select><button onClick={handleCreateDemoAccount} disabled={!newAccountName.trim()} className="w-full rounded-xl bg-primary py-3 text-[13px] font-semibold text-background disabled:cursor-not-allowed disabled:opacity-40">Create Account</button></div></Modal>}
