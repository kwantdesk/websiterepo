...
function StrategyCard({ strategy, onClick }: { strategy: Strategy; onClick: () => void }) {
  ...
  return (
    <button onClick={onClick} className="rounded-2xl border border-border bg-panel p-5 text-left transition-all hover:border-primary/20">
      ...
      <div className="mt-4 flex justify-end">{strategy.paid ? <span className="rounded-xl bg-primary px-3 py-1.5 text-[12px] font-semibold text-on-primary">Buy</span> : <span className="text-[12px] font-semibold text-primary">Clone</span>}</div>
    </button>
  );
}
    ...
    <div className="flex min-h-screen bg-background text-foreground">
      ...
      <main className="min-w-0 flex-1">
        <header className="sticky top-0 z-20 flex flex-wrap items-center gap-4 border-b border-border bg-background/95 px-6 py-4 backdrop-blur">
          ...
          <button onClick={() => setShowPublish(true)} className="flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-[13px] font-semibold text-on-primary"><Plus className="h-4 w-4" />Publish Strategy</button>
          ...
          {selected && (
            <section className="rounded-2xl border border-border bg-panel p-6">
              ...
              <div className="flex flex-wrap items-start gap-4">
                ...
                <button className="rounded-xl bg-primary px-5 py-2.5 text-[13px] font-semibold text-on-primary">{selected.paid ? `Buy for ${selected.price}` : "Clone for Free"}</button>
              </div>
      ...
      {showPublish && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setShowPublish(false)}>
          <div className="w-[600px] rounded-2xl border border-border bg-panel p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mb-5 flex gap-2">{[1, 2, 3, 4].map((step) => <button key={step} onClick={() => setPublishStep(step)} className={`h-8 w-8 rounded-full text-[12px] ${publishStep === step ? "bg-primary text-on-primary" : "bg-surface text-muted"}`}>{step}</button>)}</div>
