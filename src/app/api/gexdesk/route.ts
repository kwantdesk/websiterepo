...
import { createGexDeskFixture } from "@/lib/gexDesk";
...
export async function GET(request: NextRequest) {
  if (!getConfiguredQuantDataApiKey()) {
    if (
      ...
      return NextResponse.json(createGexDeskFixture(), {
        headers: { "Cache-Control": "private, no-store, max-age=0" },
      });
    }
    return NextResponse.json({ error: "Gexdesk is not configured." }, { status: 503 });
  }
  ...
  try {
    const payload = await getGexDeskPayload();
