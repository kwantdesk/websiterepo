...
const validViews = new Set(["classic", "state"]);
...
export async function GET(request: NextRequest) {
  ...
  const envelope = await fetchGexBotTerminal(view as "classic" | "state", ticker, category, false);
  const frame = normalizeGexBotEnvelope(envelope as typeof envelope & { frame: GexBotProfileFrame | null });
