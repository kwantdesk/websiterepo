...
import { buildOptionsSurfaceNetSeries, optionsDeltaSourceForInstrument } from "@/lib/optionsDelta";
...
const cache = new Map<string, { expiresAt: number; payload: ZeroGammaBarsReceipt }>();

type ZeroGammaBarsReceipt = {
  schemaVersion: 1;
  id: "zero-gamma-bars";
...
export async function GET(request: NextRequest) {
  ...
  if (!sourceSymbol) {
    return NextResponse.json(
      { error: "Zero Gamma Bars requires a supported futures or options-underlying family." },
      { status: 400 },
    );
  }
  ...
  if (asOfText && (!Number.isFinite(replayAsOfMs) || replayAsOfMs! <= 0 || replayAsOfMs! > receivedAtMs)) {
    return NextResponse.json({ error: "A valid non-future Zero Gamma Bars replay cutoff is required." }, { status: 400 });
  }
  ...
  try {
    ...
    if (surface.greekMode !== "GAMMA" || surface.scope !== "FRONT_EXPIRY" ||
      ...
      return NextResponse.json(
        { error: "The Zero Gamma Bars surface does not match the requested method or session." },
    ...
    const points = buildOptionsSurfaceNetSeries({ frames })
    ...
    if (!Number.isFinite(asOfMs) || asOfMs <= 0 || asOfMs > receivedAtMs + 60_000 ||
      ...
      return NextResponse.json(
        { error: "The Zero Gamma Bars receipt is not point-in-time safe." },
        { status: 502, headers: { "Cache-Control": "private, no-store" } },
      );
    }
    ...
    const payload: ZeroGammaBarsReceipt = {
      schemaVersion: 1,
      id: "zero-gamma-bars",
      displayInstrument,
      sourceSymbol,
