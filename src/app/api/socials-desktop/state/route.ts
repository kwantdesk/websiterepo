...
export async function GET(request: NextRequest) {
  ...
  try {
    const types = (request.nextUrl.searchParams.get("types") ?? "").split(",").filter(Boolean);
    const state = await createSocialsReadServiceFromEnv().state(actor.userId, {
      mineOnly: request.nextUrl.searchParams.get("mine") === "1",
      objectTypes: types,
    });
