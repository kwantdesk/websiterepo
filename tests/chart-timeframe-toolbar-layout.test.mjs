...
test("workspaces and the chart timeframe selector remain centred", () => {
  assert.match(workspace, /kwant-chart-command-deck[^\n]*grid-cols-\[minmax\(0,1fr\)_auto_minmax\(0,1fr\)\]/);
  assert.match(workspace, /col-start-2 flex h-7 shrink-0 items-center justify-self-center/);
...
test("status stays left while source, time, and chart actions stay right-aligned", () => {
